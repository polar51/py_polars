...
프로젝트만의 독립된 공간(.venv)을 만들어 설치 후 사용
```powershell
venv(가상환경) 설치: py -m venv .venv
venv(가상환경) 접속: .\.venv\Scripts\Activate.ps1
polars 설치: pip install polars
파이썬 실행: python main.py
```
...
## TODO - 추후 변경점
1. 도입부 ( API, 로그프레소, CSV 파일 등등) 변경
2. 알고리즘 공통 로직으로 변경해서 불러와서 사용하기만 가능하게
3. 송신부 ( DB 쓰기, 파일 내뱉기, API) 변경
4. 쉘 스크립트를 이용해서 지정된 시간에 배치가 돌게끔 변경
5. 배치가 실행되는 도중 확인 할 수 있게 날짜별로 로그를 쌓으면 좋을듯?
